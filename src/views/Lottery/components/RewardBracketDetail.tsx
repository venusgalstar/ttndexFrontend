  ...
  return (
    <Flex flexDirection="column">
      {isLoading ? (
        ...
        <Text bold color={isBurn ? 'failure' : 'white'}>
          {getRewardText()}
        </Text>
      )}
      <>
        {isLoading || cakeAmount.isNaN() ? (
          ...
          <Balance fontSize="20px" bold unit=" TTNP" value={getBalanceNumber(cakeAmount)} decimals={0} />
        )}
        {isLoading || cakeAmount.isNaN() ? (
          <>
          ...
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(cakeAmount.div(parseInt(numberWinners, 10)), 18, 2)} TTNP {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
